...
from utilitis import AllMyParsHere as AllMyPars
import sys
import os
import numpy as np
import pandas as pd
...
sns.despine()
from ephys import ephys_features as ephys_ft
from ephys import ephys_extractor as efex
from ephys import extraEhpys_PV
from ephys.extraEhpys_PV import extract_sweep_feature
...
import glob
from utilitis.debugHelpers import debugInfo
from utilitis.AnalysisMethods import (
    loadYAML,
    filterDatSeries,
    calculateConnectionTraces,
    cleanASCfile,
)
from utilitis import fitFuncs
from utilitis.patchviewObjects import *
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, app, parent=None):
        ...
        self.setWindowIcon(pg.QtGui.QIcon("PatchViewer.ico"))
    ...
    def make_layout(self):
        ...
        # self.selectedFiles_splitter = QtGui.QSplitter(QtCore.Qt.Horizontal)
        # tree_splitter.addWidget(self.selectedFiles_splitter)
    ...
    def event_viewSingleEvents(
        ...
        # wTime2 = self.events.time[upStrokeIdx: donwstrokeIdx]
        # wdata2 = self.events.data[upStrokeIdx: donwstrokeIdx]
    ...
    def updateTreeMorphView(self, fname):
        ...
        if hasattr(neurons, "neurons"):
            ...
            # ['Summary','Axon', 'Apic dendrite', 'Basal dendrite']
            ## get summary
            # self.splitViewTab_morph.tables['Summary'].clear()
    ...
    def add_toolbar(self):
        ...
        self.tooglesettingsAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/settings.png"), "Global settings"
        )
        ...
        self.toogleMouseModePanAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/rectangle.png"), "Mouse Mode"
        )
        ...
        self.toogleDetectSpikeAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/spikes.png"), "Firing pattern"
        )
        ...
        self.exportAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/connection2.png"), "Connectivtiy"
        )
        ...
        self.importSlicetAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/neuron.png"), "Import slice image"
        )
        # self.exportAction.setShortcut("Alt+e")
        ...
        self.gpAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/GP1.png"), "Coupling ratio"
        )
        # self.exportAction.setShortcut("Alt+e")
        ...
        self.gpAction2 = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/GPcorr.png"), "correlations"
        )
        # self.exportAction.setShortcut("Alt+e")
        ...
        self.importTreeAction = pg.QtGui.QAction(
            pg.QtGui.QIcon("Data/icons/tree.png"), "Import ASC file"
        )
        # self.exportAction.setShortcut("Alt+e")
    ...
    def MouseMode_clicked(self):
        if self.MouseMode == pg.ViewBox.RectMode:
            ...
            self.toogleMouseModePanAction.setIcon(
                pg.QtGui.QIcon("Data/icons/navigation.png")
            )
            self.toogleMouseModePanAction.setStatusTip("Your Mouse is in pan mode")
        else:
            self.MouseMode = pg.ViewBox.RectMode
            self.toogleMouseModePanAction.setIcon(
                pg.QtGui.QIcon("Data/icons/rectangle.png")
            )
    ...
    def add_menubar(self):
        ...
        self.AnaAction1 = pg.QtGui.QAction("&Firing pattern analysis")
        self.AnaAction1.setStatusTip(
            "Select a firing pattern series to extract ephy features"
        )
        self.AnaAction1.triggered.connect(self.extraEphyAction_clicked)
        self.AnalysisMenu.addAction(self.AnaAction1)
        ...
        self.AnaAction5 = pg.QtGui.QAction("&Gap junction (Current tree)")
        self.AnaAction5.triggered.connect(self.checkGapJunctionAction_clicked)
        self.AnaAction5.setStatusTip("Check gap connections for current series")
        self.AnalysisMenu.addAction(self.AnaAction5)

        self.AnaAction2 = pg.QtGui.QAction("&Event detection")
        self.AnaAction2.setStatusTip("Detect spontaneous EPSPs")
        ...
        self.VisualAction5 = pg.QtGui.QAction("&FP for all slice")
        self.VisualAction5.setStatusTip(
            "Selected traces for all cells in current slice"
        )
        self.VisualAction5.triggered.connect(
            self.sliceFPView_plot
        )  ## fine tune firing pattern plotting with user define parameters
        self.VisualMenu.addAction(self.VisualAction5)
        ...
        self.batchFPana = True  ## by default, do not do batch processing
        self.OptionAction7 = pg.QtGui.QAction(
            "Firing pattern batch analysis", self, checkable=True
        )
        # OptionAction1.setShortcut("Ctrl+a")
    ...
    def getDatAndePhyFeatures(self, seriesIdx0):
        ...
        # stimInfo=[]
        # stimTime, stimData, stimInfo0= self.currentPulseTree.bundle.stim(seriesIdx)    ## assume only 1 channel is stimulating
        # stimInfo.append(stimInfo0)
    ...
    def getEphyFeatures(self):
        ...
        self.updateEphyTable(
            self.EphyFeaturesObj.Cell_Features,
    ...
    def checkFP_action_clicked(self):
        ...
        for file_idx, bundle in enumerate(bundleFiles):
            ...
            if self.batchFPana:
                ...
                data = np.squeeze(bundle.getSeriesData(sel))
                # try:
                ephObj = self.doFPanalysis(time, data, sel, stimInfo, cellName)
                # pdb.set_trace()
                self.updateEphyTable(
                    ephObj.Cell_Features,
                    self.splitViewTab_FP.tables["Cell features"],
                    ephObj.title,
                )
                print(cellName, "done!")
